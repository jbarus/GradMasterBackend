...
public class ProblemController {
    ...
    public ResponseEntity<?> publishProblemToQueue(@PathVariable UUID contextId) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new Response<>(CalculationStartStatus.NO_SUCH_CONTEXT));
        } catch (MissingDataException e) {
            return ResponseEntity.badRequest().body(new Response<>(e.getStatus()));
        } catch (Exception e){
            return ResponseEntity.internalServerError().body(new Response<>(CalculationStartStatus.INTERNAL_ERROR));
        }
    }
}
